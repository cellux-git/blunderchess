...
impl Eval {
    pub(crate) fn eval_king_safety(&self, board: &Board, color: Color, king_sq: Square, pawns_bb: u64, enemy_bb: u64) -> i32 {
    ...
    pub(crate) fn eval_king_opposition(&self, board: &Board, color: Color, mg: &mut i32, eg: &mut i32) {
        ...
        if enemy_majors != 0 { return; }

        let df = (kf - ekf).abs();
        let dr = (kr - ekr).abs();
        if !((df == 2 && dr == 0) || (df == 0 && dr == 2)) { return; }
        ...
        if between & board.occupancy() != 0 { return; }
